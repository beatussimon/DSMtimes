...
from .models import Article, Category, SubscriptionPlan, Comment, NewsletterSubscription, UserProfile

def home(request):
    featured_article = Article.objects.filter(status='published', is_premium=False).order_by('-published_at').first()
    latest_articles = Article.objects.filter(status='published').order_by('-published_at')[:12]
    trending_articles = Article.objects.filter(status='published').order_by('-views_count')[:5]
